// components/admin/tabs/StagesTab.tsx
"use client";

import * as React from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import SpotForm from "@/components/stages/SpotForm";
import { Button } from "@/components/ui/button";
import { Switch } from "@/components/ui/switch";
import { useToast } from "@/hooks/use-toast";

type SpotRow = {
  id: string; title: string; companyName: string; city?: string | null;
  countryCode: string; approved: boolean;
};
...
export default function StagesTab() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const mapped = (json.spots ?? []).map((s: any) => ({
        ...
        city: s.city, countryCode: s.countryCode, approved: s.approved,
      }));
      setRows(mapped);
  ...
  const toggleApprove = async (id: string, value: boolean) => {
    ...
    if (!res.ok) return toast({ title: "Erreur", description: "MàJ impossible", variant: "destructive" });
  ...
  const remove = async (id: string) => {
    ...
    if (!res.ok) return toast({ title: "Erreur", description: "Suppression impossible", variant: "destructive" });
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Modération (approuver / supprimer)</CardTitle>
        </CardHeader>
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td className="px-4 py-4" colSpan={6}>Chargement...</td></tr>
              ) : rows.length === 0 ? (
                <tr><td className="px-4 py-4" colSpan={6}>Aucun point à afficher.</td></tr>
              ) : (
                rows.map((r) => (
